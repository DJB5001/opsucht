...
function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('urbanexUser');
    const savedUsers = localStorage.getItem('urbanexUsers');
    const savedOrders = localStorage.getItem('urbanexOrders');
    const savedAbsences = localStorage.getItem('urbanexAbsences');
    
    if (savedUser) setUser(JSON.parse(savedUser));
    if (savedUsers) setUsers(JSON.parse(savedUsers));
    else {
      setUsers(DEFAULT_USERS);
      localStorage.setItem('urbanexUsers', JSON.stringify(DEFAULT_USERS));
    }
  ...
  useEffect(() => {
    localStorage.setItem('urbanexUsers', JSON.stringify(users));
  }, [users]);

  useEffect(() => {
    localStorage.setItem('urbanexOrders', JSON.stringify(orders));
  }, [orders]);

  useEffect(() => {
    localStorage.setItem('urbanexAbsences', JSON.stringify(absences));
  }, [absences]);
  ...
  const handleLogin = (username: string, password: string): boolean => {
    ...
    if (foundUser) {
      setUser(foundUser);
      localStorage.setItem('urbanexUser', JSON.stringify(foundUser));
  ...
  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('urbanexUser');
